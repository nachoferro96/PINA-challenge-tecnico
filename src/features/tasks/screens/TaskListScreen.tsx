...
import {
  TaskLoadingState,
  TaskScreenState,
} from '../components/TaskScreenState';
...
export function TaskListScreen({
  ...
  const colorStyles = StyleSheet.create({
    scenarioLabel: { color: colors.text },
  });
  const isLoading = state.loadStatus === 'loading';
  const hasLoadedTasks =
    state.loadStatus === 'success' && state.tasks.length > 0;
  ...
  const header = (
    <View>
      {Platform.OS === 'android' || hasLoadedTasks ? (
        <View style={styles.topRow}>
          ...
          {Platform.OS === 'android' ? (
            ...
            >
              <Text style={[styles.textButtonLabel, { color: colors.accent }]}>
                Estadísticas
              </Text>
            </Pressable>
          ) : null}
      ...
      {hasLoadedTasks ? <SummaryStrip statistics={state.statistics} /> : null}

      <SearchControl value={state.filters.query} onChange={setQueryFilter} />

      <View
        style={[styles.filters, fontScale >= 1.2 && styles.filtersLargeText]}
      >
        <View
          style={[
            styles.filterSlot,
            fontScale >= 1.2 && styles.filterSlotLargeText,
          ]}
        >
          <FilterControl
            label="Estado"
  ...
  return (
    <FlatList
      ...
      contentContainerStyle={[
        ...
        {
          paddingTop:
            Platform.OS === 'ios' ? spacing.md : insets.top + spacing.md,
